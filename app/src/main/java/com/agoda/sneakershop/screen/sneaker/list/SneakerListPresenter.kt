...
class SneakerListPresenter(private val interactor: SneakerListInteractor)
    ...
    override fun defaultViewModel(): SneakerListViewModel =
            SneakerListViewModel(items = listOf(), loading = false, error = null, query = "")
    ...
    override fun fetchSneakers(query: String) {
       ...
       disposables += interactor.getSneakers(query).subscribe({
            success ->
            viewModel = viewModel.copy(loading = false, items = success, error = null)
        }, {
            error ->
            viewModel = viewModel.copy(loading = false, items = listOf(), error = error)
        })

   /*     disposables += interactor.getSneakers(query)
                .map { viewModel.copy(loading = false, items = it, error:Any? = null) }
                .onErrorReturn { viewModel.copy(loading = false, items = listOf(), error = error) }
                .startWith(viewModel.copy(
                        loading = true, query = query) })
